...
def test_freq_columns():
    df = pd.DataFrame(
        {
            "x": ["a", "c", "b", "g", "h", "a", "g", "a"],
            "y": [10, 30, 20, 40, 50, 10, 40, 10],
        }
    )
    actual = pandastools.freq_columns(df)
    rows = [
        dict(n=3, N=3, r=0.375, R=0.375),
        dict(n=2, N=5, r=0.250, R=0.625),
        dict(n=1, N=6, r=0.125, R=0.750),
    ...
    expected = {
        "x": pd.DataFrame(rows, index=["a", "g", "c", "b", "h"]),
        "y": pd.DataFrame(rows, index=[10, 40, 30, 20, 50]),
    }
    assert isinstance(actual, dict)
    for column in actual:
        pd.testing.assert_frame_equal(actual[column], expected[column])
